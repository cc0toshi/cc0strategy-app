...
import { NFTMarketplace } from '@/components/marketplace';
...
// FeeDistributor ABI for fetching total rewards
...
// FeeDistributor V2 addresses (from contracts.ts)
const FEE_DISTRIBUTOR_ADDRESSES = {
  base: '0x498bcfdbd724989fc37259faba75168c8f47080d' as `0x${string}`,
  ethereum: '0xdcfb59f2d41c58a1325b270c2f402c1884338d0d' as `0x${string}`,
} as const;
...
function TokenHeader({ token, priceData, totalRewards, rewardsLoading }: { token: TokenInfo | null; priceData: PriceData | null; totalRewards: string | null; rewardsLoading: boolean }) {
    ...
    <div className="border-2 border-white p-4 md:p-6 mb-6">
      <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-8">
        ...
        <div className="flex flex-wrap gap-4 md:gap-8 md:ml-auto">
          ...
          <div>
            <div className="text-neutral-500 text-xs mb-1">TOTAL REWARDS</div>
            <div className="font-mono text-green-400">{rewardsLoading ? '...' : (totalRewards ? `${totalRewards} WETH` : '—')}</div>
          </div>
        </div>
...
// Fetch token price and rewards from cache (single API call)
  useEffect(() => {
    if (!selectedToken) {
    ...
    const fetchCachedData = async () => {
      ...
      try {
        const res = await fetch(`/api/cache/token?address=${selectedToken.address}`);
        if (res.ok) {
          const data = await res.json();
          // Set price data
          if (data.market) {
            setTokenPriceData(data.market);
          }
          // Set rewards
          if (data.rewards) {
    ...
    fetchCachedData();
    // Refresh every 30 seconds (cache updates every 30-60s)
  ...
  return (
    <div className="w-full px-4 md:px-8 lg:px-12 py-6 md:py-12">
      ...
      {/* Token Header */}
      <TokenHeader token={selectedToken} priceData={tokenPriceData} totalRewards={totalRewards} rewardsLoading={rewardsLoading} />
      ...
      <div className="grid lg:grid-cols-12 gap-6 lg:gap-8">
        ...
        {selectedToken?.nft_collection && (
          <NFTMarketplace
            collectionAddress={selectedToken.nft_collection}
            collectionName={selectedToken.name}
